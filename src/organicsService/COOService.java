	...
	public LinkedList<String> transformMoleFormula(String moleFormula) {
		...
		if(oNumber==2) {
			if(cNumber*2==hNumber&&cNumber>=3) {
				...
				bonds.add("C2O2"+BondType.CO120DoubleBond);
				...
				bonds.add("O1C4"+BondType.COTeSingleBond);
				bonds.add("C3O2"+BondType.CO120DoubleBond);
				for(int i=1;i<=cNumber;i++) {
					if(i==1) {
						for(int j=0;j<2;j++) {
				...
				bonds.add("O1C4"+BondType.COTeSingleBond);
				bonds.add("C3O2"+BondType.CO120SingleBond);
				for(int i=1;i<=cNumber;i++) {
				...
				bonds.add("O1C8"+BondType.CO120SingleBond);
				bonds.add("C7O2"+BondType.CO120DoubleBond);
				for(int i=1;i<=cNumber;i++) {
